...
matplotlib.use('TkAgg')  # 或 'Qt5Agg'，視你的環境而定
...
def random_pick_subset(dataset, subset_size):
    indices = list(range(len(dataset)))
    random.shuffle(indices)
    train_dataset = torch.utils.data.Subset(dataset, indices[:subset_size])
...
transform = transforms.Compose([
    # transforms.Resize((320, 320)),  # adjust size for MobileNetV3
...
train_dataset = random_pick_subset(train_dataset, 1000)  # random pick 100 images
